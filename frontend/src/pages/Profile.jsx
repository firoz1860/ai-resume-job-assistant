      ...
      <main className="flex-1 py-8 md:py-12">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-8">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-ink">Profile</h1>
              <p className="text-sm text-muted mt-1">{user?.email}</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex gap-2 w-full lg:w-auto">
              <button onClick={restoreSaved} className="btn-secondary text-sm">Load Saved</button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[0.8fr_1.2fr] gap-6 items-start">
            <aside className="card p-5 sm:p-6 lg:sticky lg:top-24">
              <p className="text-xs font-semibold text-muted uppercase">Profile Completion</p>
              <p className="text-4xl font-extrabold text-navy-900 mt-2">{profileCompletion}%</p>
              ...
              <div className="mt-5 space-y-2 text-sm">
                <div className="flex justify-between border border-border rounded-lg p-3 bg-surface"><span>Target role</span><strong>{savedProfile.targetRole || 'Not set'}</strong></div>
                <div className="flex justify-between border border-border rounded-lg p-3 bg-surface"><span>Dream company</span><strong>{savedProfile.dreamCompany || 'Not set'}</strong></div>
                <div className="flex justify-between border border-border rounded-lg p-3 bg-surface"><span>Job type</span><strong>{savedProfile.jobType || 'Not set'}</strong></div>
              </div>
            </aside>
              ...
              {!loading && (
                <section className="card p-5 sm:p-6 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <button className="btn-primary justify-center" disabled={saving}>{saving ? 'Saving...' : 'Save Profile'}</button>
