...
export default function Roadmap() {
  ...
  const submit = async (e) => { e.preventDefault(); setLoading(true); try { setResult(await roadmapApi.create(form)); } finally { setLoading(false); } };
  return <div className="min-h-screen flex flex-col"><Navbar /><main className="flex-1 py-8 md:py-12"><div className="max-w-6xl mx-auto px-4 sm:px-6">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[0.8fr_1.2fr] gap-6 items-start">
      ...
      <div className="space-y-4">{result ? result.plan.map((week) => <div key={week.week} className="card p-5"><p className="text-xs text-muted uppercase font-semibold">Week {week.week}</p><h2 className="font-bold text-ink mb-2">{week.focus}</h2><div className="flex flex-wrap gap-2 mb-3">{week.topics.map((t) => <span key={t} className="text-xs border border-border rounded-lg px-2 py-1">{t}</span>)}</div>{week.dailyTasks.map((task) => <p key={task} className="text-sm bg-surface border border-border rounded-lg p-2 mb-2">{task}</p>)}<p className="text-sm font-semibold text-accent">{week.miniProject}</p></div>) : <div className="card p-6 text-sm text-muted">Your roadmap timeline will appear here.</div>}</div>
    </div>
  </div></main><Footer /></div>;
}
